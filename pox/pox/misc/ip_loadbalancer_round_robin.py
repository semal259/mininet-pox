...
class iplb (object):
  ...
  def _pick_server (self, key, inport):
    #Pick a server for a (hopefully) new connection in round robin fashion
    global selected_server
    servers = self.live_servers.keys()
    total_servers = len(self.live_servers)
    if(selected_server == total_servers):
        selected_server = 0
    available_server = servers[selected_server]
    selected_server += 1
